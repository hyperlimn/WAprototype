...
export class ServiceSupervisor {
  ...
  status() { return { services: MANAGED_SERVICES.map((service) => ({ ...service, owned: this.isOwned(service.id), pid: this.children.get(service.id)?.pid ?? null })),
    supervisorPid: process.pid, lastRestart: [...this.runs].reverse().find((run) => run.commandId === "runtime.restart-all" || run.commandId === "runtime.resume-save") ?? null }; }
    ...
    if (!this.diagnosticsCache || Date.now() >= this.diagnosticsCache.expiresAt) {
      ...
      try { const memory = await this.requestJson("http://127.0.0.1:8787/api/memory/status");
        const diagnostics = memory.manifestPersistence; manifestPersistence = diagnostics
          ? { ...diagnostics, healthy: diagnostics.lastFailure === null } : { healthy: false, reason: "No active manifest writer diagnostics" }; }
      catch (error) { manifestPersistence = { healthy: false, reason: error instanceof Error ? error.message : String(error) }; }
      this.diagnosticsCache = { expiresAt: Date.now() + 3_000,
        value: { duplicateProtoUniverseInstances: processDiscoveryWarning ? -1 : staleRepoProcesses(this.root, processes, keep).length,
          processDiscoveryWarning, manifestPersistence } };
    }
    return { ...this.status(), ...this.diagnosticsCache.value };
  }
  ...
  private async verifySingleInstance(run: ServiceRun): Promise<void> {
    ...
    this.append(run, "Single-instance verification passed\n");
  }
