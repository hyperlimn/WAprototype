...
export const targetSchema = z.object({ kind: targetKind, seed, id: z.string().min(1).max(300).optional(), x: z.number().finite().optional(),
  y: z.number().finite().optional(), radius: z.number().finite().min(0).optional(), tick: z.number().int().min(0).optional(),
  sequence: z.union([z.number().int().min(0), z.string().regex(/^event-[0-9a-f]{8}$/)]).optional(),
...
export const historySchema = z.object({ seed, sinceTick: z.number().int().min(0).optional(), untilTick: z.number().int().min(0).optional(),
  ...
  entityId: z.union([z.number().int().min(0), z.string().regex(/^entity-(?:[0-9a-f]{8}|[0-9a-f]{16})$/)]).optional(), relationshipId: z.string().min(1).max(300).optional(), limit,
