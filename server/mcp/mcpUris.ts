...
export const entityUri = (seed: string, id: number | string): string => `${universeUri(seed)}/entity/${component(String(id))}`;
...
export const eventUri = (seed: string, tick: number, sequence: number | string): string => `${universeUri(seed)}/event/${tick}/${component(String(sequence))}`;
...
export type ParsedProtoUniverseUri =
  ...
  | { kind: "entity"; seed: string; id: number | string }
  | { kind: "relationship"; seed: string; id: string }
  | { kind: "checkpoint"; seed: string; tick: number }
  | { kind: "event"; seed: string; tick: number; sequence: number | string }
...
export function parseProtoUniverseUri(value: string): ParsedProtoUniverseUri {
  ...
  if (parts[2] === "entity" && (/^\d+$/.test(parts[3] ?? "") || /^entity-(?:[0-9a-f]{8}|[0-9a-f]{16})$/.test(parts[3] ?? "")) && parts.length === 4)
    return { kind: "entity", seed, id: /^\d+$/.test(parts[3]) ? Number(parts[3]) : parts[3] };
  ...
  if (parts[2] === "event" && /^\d+$/.test(parts[3] ?? "") && (/^\d+$/.test(parts[4] ?? "") || /^event-[0-9a-f]{8}$/.test(parts[4] ?? "")) && parts.length === 5)
    return { kind: "event", seed, tick: Number(parts[3]), sequence: /^\d+$/.test(parts[4]) ? Number(parts[4]) : parts[4] };
