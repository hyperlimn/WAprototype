...
export function registerMcpResources(server: McpServer, gateway: Gateway, options: { bookmarks?: boolean; universe?: boolean;
  entities?: boolean; relationships?: boolean; checkpoints?: boolean; events?: boolean; regions?: boolean; presentMoment?: boolean } = {}): void {
  if (options.universe ?? true) server.registerResource("universe", template("protouniverse://universe/{seed}"), { title: "ProtoUniverse archive or live orientation", description: "Compact universe identity, archive metadata, and perception orientation.", mimeType: "application/json" },
  ...
  if (options.entities ?? true) server.registerResource("entity", template("protouniverse://universe/{seed}/entity/{id}"), { title: "ProtoUniverse entity", description: "Authoritative entity record with derived inspection, context, lineage, and relationship links.", mimeType: "application/json" },
    async (uri, variables) => { const seed = String(variables.seed), rawId = String(variables.id);
      const id: number | string = /^\d+$/.test(rawId) ? Number(rawId) : rawId;
      if (!(typeof id === "number" ? Number.isInteger(id) && id >= 0 : /^entity-(?:[0-9a-f]{8}|[0-9a-f]{16})$/.test(id))) throw new Error("invalid entity id");
  ...
  if (options.relationships ?? true) server.registerResource("relationship", template("protouniverse://universe/{seed}/relationship/{id}"), { title: "ProtoUniverse relationship",
    description: options.presentMoment ? "Present relationship record and current parent context." : "Authoritative relationship record, parent resources, context, and rupture history.", mimeType: "application/json" },
  ...
  if (options.checkpoints ?? true) server.registerResource("checkpoint", template("protouniverse://universe/{seed}/checkpoint/{tick}"), { title: "ProtoUniverse checkpoint", description: "Canonical observational checkpoint; not simulation resume authority.", mimeType: "application/json" },
  ...
  if (options.events ?? true) server.registerResource("event", template("protouniverse://universe/{seed}/event/{tick}/{sequence}"), { title: "ProtoUniverse occurrence", description: "Persisted authoritative occurrence with connected resource URIs.", mimeType: "application/json" },
    async (uri, variables) => { const seed = String(variables.seed), tick = Number(variables.tick), rawSequence = String(variables.sequence);
      const sequence: number | string = /^\d+$/.test(rawSequence) ? Number(rawSequence) : rawSequence;
      if (!(typeof sequence === "number" ? Number.isInteger(sequence) && sequence >= 0 : /^event-[0-9a-f]{8}$/.test(sequence))) throw new Error("invalid event id");
  ...
  if (options.regions ?? true) server.registerResource("region", template("protouniverse://universe/{seed}/region/{x}/{y}/{radius}"), { title: "ProtoUniverse inferred region", description: "Geometric regional perception. This is inferred context, not authoritative cluster identity.", mimeType: "application/json" },
