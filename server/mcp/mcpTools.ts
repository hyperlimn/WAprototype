...
type Faculty = "orient" | "inspect" | "context" | "anomalies" | "similar" | "compare" | "changes" | "list_universes" | "history" | "checkpoints" | "checkpoint" | "human_view" | "law_epoch" | "laws" | "law_history" | "law_inspect" | "counterfactual_interventions" | "counterfactual_create" | "counterfactual_status" | "counterfactual_compare" | "counterfactual_inspect" | "counterfactual_terminate";
...
export function registerMcpTools(server: McpServer, gateway: Gateway, options: McpToolOptions = {}): void {
  ...
  const cfVector=z.object({kind:z.enum(["entity-impulse","cluster-impulse","entity-displace","cluster-displace"]),entityId:z.number().int().min(0),x:z.number().finite(),y:z.number().finite()}).strict();
  const cfScalar=z.object({kind:z.enum(["cluster-expand","cluster-compress","cluster-spin-clockwise","cluster-spin-counterclockwise"]),entityId:z.number().int().min(0),magnitude:z.number().positive().max(4)}).strict();
  const cfSever=z.object({kind:z.literal("relationship-sever"),relationshipId:z.string().regex(/^\d+:\d+$/)}).strict();
  if(enabled("counterfactual_interventions"))server.registerTool("counterfactual_interventions",{title:"List counterfactual interventions",description:"Enumerate bounded temporary-branch interventions. No primary state is changed.",inputSchema:z.object({}).strict(),outputSchema:genericOutput,annotations:readOnly},async()=>output({schemaVersion:"counterfactual-intervention/2",maximumVectorMagnitude:4,maximumDisplacement:500,clusterMaximumMembers:32,kinds:["entity-impulse","cluster-impulse","entity-displace","cluster-displace","cluster-expand","cluster-compress","cluster-spin-clockwise","cluster-spin-counterclockwise","relationship-sever"]},"Supported bounded counterfactual interventions."));
  if(enabled("counterfactual_create"))server.registerTool("counterfactual_create",{title:"Create temporary counterfactual",description:"Capture the connected primary continuation internally and create one isolated temporary branch.",inputSchema:z.union([cfVector,cfScalar,cfSever]),outputSchema:genericOutput,annotations:observerWrite},async(args)=>output(await gateway.post("/api/counterfactual/create",args),"Temporary counterfactual creation requested; primary universe unchanged."));
  if(enabled("counterfactual_status"))server.registerTool("counterfactual_status",{title:"Counterfactual status",description:"Read active branch identity, synchronization, hashes, metrics, and bounded session summaries.",inputSchema:z.object({}).strict(),outputSchema:genericOutput,annotations:readOnly},async()=>output(await gateway.get("/api/counterfactual/status"),"Counterfactual branch status."));
  if(enabled("counterfactual_compare"))server.registerTool("counterfactual_compare",{title:"Compare counterfactual",description:"Read bounded primary-versus-branch divergence metrics, optionally focused on one mapped target.",inputSchema:z.object({entityId:z.number().int().min(0).optional(),relationshipId:z.string().max(120).optional()}).strict().refine(v=>!(v.entityId!==undefined&&v.relationshipId!==undefined)),outputSchema:genericOutput,annotations:readOnly},async(args)=>output(await gateway.post("/api/counterfactual/compare",args),"Bounded primary-versus-branch comparison."));
  if(enabled("counterfactual_inspect"))server.registerTool("counterfactual_inspect",{title:"Inspect counterfactual",description:"Inspect one mapped entity or relationship, or a bounded branch sample.",inputSchema:z.object({entityId:z.number().int().min(0).optional(),relationshipId:z.string().max(120).optional()}).strict().refine(v=>!(v.entityId!==undefined&&v.relationshipId!==undefined)),outputSchema:genericOutput,annotations:readOnly},async(args)=>output(await gateway.post("/api/counterfactual/inspect",args),"Bounded branch observation."));
  if(enabled("counterfactual_terminate"))server.registerTool("counterfactual_terminate",{title:"Terminate counterfactual",description:"Terminate only the active temporary branch and retain a bounded session summary.",inputSchema:z.object({}).strict(),outputSchema:genericOutput,annotations:observerWrite},async()=>output(await gateway.post("/api/counterfactual/terminate",{}),"Temporary counterfactual termination requested."));
