...
Runtime hardening extends that transaction with same-repository stale-process cleanup. Only command lines containing this exact repository plus a reviewed ProtoUniverse entry point qualify; every PID is re-verified before its tree can be terminated, the active supervisor and Codex are excluded, and an unknown process on a required port causes a fail-closed result. The Machine Interface reports supervisor/service PIDs, duplicate-runtime count, and manifest-writer health. Manifest writes remain atomic and retry bounded transient Windows file locks without unlinking the previous valid manifest.
...
Long-run observer costs are instrumented in the Machine Interface panel. Renderer frame time and rendered/candidate edge counts update at a one-second observational cadence. Canonical machine snapshots retain authoritative tick metadata and are published every 15 seconds rather than every 5 seconds; one-second heartbeats continue to report the live tick. Expensive sidebar diagnostics pause while their panel is collapsed, and offscreen relationship segments are culled only from human rendering, never authoritative state.
